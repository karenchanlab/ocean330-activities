...
server <- function(input, output, session) {
    ...
    output$plot1 <- renderPlotly({
      ...
      df0 <- cbind(pH, as.data.frame(res0))
      names(df0) <- c("pH","CO2","HCO3","CO3")
      
      K1v <- as.numeric(K1(S=input$sal, T=input$temp, P=input$pres, pHscale="T"))
      K2v <- as.numeric(K2(S=input$sal, T=input$temp, P=input$pres, pHscale="T"))
      res <- speciation(K1=K1v, K2=K2v, K3=NULL, pH, conc=1)
      df <- cbind(pH, as.data.frame(res))
      names(df) <- c("pH","CO2","HCO3","CO3")
      
      p <- ggplot(df0) +
        geom_line(aes(pH, CO2), color="grey") +
        geom_line(aes(pH, HCO3), color="grey", linetype="dashed") +
        geom_line(aes(pH, CO3), color="grey", linetype="dotted") +
        geom_line(data=df, aes(pH, CO2, linetype="CO₂"), color="darkred") +
        geom_line(data=df, aes(pH, HCO3, linetype="HCO₃⁻"), color="darkred") +
        geom_line(data=df, aes(pH, CO3, linetype="CO₃²⁻"), color="darkred") +
        scale_y_continuous(name="Relative concentration", breaks=seq(0, 1, 0.2)) +
        scale_x_continuous(name="pH(tot)", breaks=seq(2, 12, 0.5), expand = c(0,0)) + 
        scale_linetype_manual(values = c("CO₂" = "solid", "HCO₃⁻" = "dashed", "CO₃²⁻" = "dotted")) + 
        theme_minimal()
